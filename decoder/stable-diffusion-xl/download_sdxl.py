...
REPO_FOLDER  = "decoder/stable-diffusion-xl"   # carpeta dentro del repo
LOCAL_DIR    = os.path.join(tempfile.gettempdir(), "stable-diffusion-xl-base-1.0")
...
print(f"[1/4] Descargando {HF_MODEL_ID} → {LOCAL_DIR}  (~7 GB)")
...
subprocess.run(["git", "commit", "-m", f"feat: add weights for stable-diffusion-xl-base-1.0 ({m.approxSize})"], check=True)
