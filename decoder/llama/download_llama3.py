...
download_llama3.py
══════════════════════════════════════════════════════════════
Descarga los pesos de meta-llama/Meta-Llama-3-8B-Instruct desde HuggingFace
...
REPO_FOLDER  = "decoder/llama"   # carpeta dentro del repo
LOCAL_DIR    = os.path.join(tempfile.gettempdir(), "llama-3-8b-instruct")
...
print(f"[1/4] Descargando {HF_MODEL_ID} → {LOCAL_DIR}  (~16 GB)")
...
subprocess.run(["git", "commit", "-m", f"feat: add weights for llama-3-8b-instruct ({m.approxSize})"], check=True)
subprocess.run(["git", "push"], check=True)

print(f"\n✅  Pesos de llama-3-8b-instruct subidos correctamente.")
