...
download_phi3.py
══════════════════════════════════════════════════════════════
Descarga los pesos de microsoft/Phi-3-mini-4k-instruct desde HuggingFace
...
REPO_FOLDER  = "decoder/phi3"   # carpeta dentro del repo
LOCAL_DIR    = os.path.join(tempfile.gettempdir(), "phi-3-mini-4k-instruct")
...
print(f"[1/4] Descargando {HF_MODEL_ID} → {LOCAL_DIR}  (~7.6 GB)")
...
subprocess.run(["git", "commit", "-m", f"feat: add weights for phi-3-mini-4k-instruct ({m.approxSize})"], check=True)
subprocess.run(["git", "push"], check=True)

print(f"\n✅  Pesos de phi-3-mini-4k-instruct subidos correctamente.")
