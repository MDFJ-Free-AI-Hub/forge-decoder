...
══════════════════════════════════════════════════════════════
Descarga los pesos de deepseek-ai/DeepSeek-V3 desde HuggingFace
...
REPO_FOLDER  = "decoder/deepseek-v3"   # carpeta dentro del repo
LOCAL_DIR    = os.path.join(tempfile.gettempdir(), "deepseek-v3")
...
print(f"[1/4] Descargando {HF_MODEL_ID} → {LOCAL_DIR}  (~641 GB (FP8))")
...
subprocess.run(["git", "commit", "-m", f"feat: add weights for deepseek-v3 ({m.approxSize})"], check=True)
subprocess.run(["git", "push"], check=True)

print(f"\n✅  Pesos de deepseek-v3 subidos correctamente.")
